...
//   }
//
//   Future<Map<String, String>> extractCardDetails(
...
//           final lineText = line.text;
//           if (RegExp(r'\d{4} \d{4} \d{4} \d{4}').hasMatch(lineText) && centerCrop.contains(blockCenter)) {
//             cardNumber = lineText;
//           }
//           if (RegExp(r'\d{2}/\d{2}').hasMatch(lineText) && centerCrop.contains(blockCenter)) {
//             expiryDate = lineText;
//           }
//         }
//       }
...
//   Uint8List _concatenatePlanes(List<Plane> planes) {
//     final length =
//         planes.fold(0, (prev, element) => prev + element.bytes.length);
//     final bytes = Uint8List(length);
//     int offset = 0;
//     for (var plane in planes) {
//       bytes.setRange(offset, offset + plane.bytes.length, plane.bytes);
